...
export function RouteMap({
  ...
  const focusActive = useCallback(() => {
    ...
    /*
     * **이미 보이는 구간이면 지도를 건드리지 않는다.**
     *
     * 단계를 넘길 때마다 지도가 제자리를 잡아버리면, 방금 손으로 맞춰 놓은 화면이
     * 매번 날아간다. 어디를 보고 있었는지 다시 찾아야 해서 오히려 성가시다.
     * 그래서 지금 화면에 그 구간이 들어와 있으면 그대로 둔다.
     *
     * 다만 **처음 열 때와 화면 밖으로 나간 구간**은 옮겨준다.
     * 안 그러면 147m 걷기처럼 짧은 구간이 4km 화면 어딘가에 숨어서 아예 안 보인다.
     */
    if (!firstFocusRef.current) {
      const view = map.getBounds()
      if (view && view.contain(bounds.getSouthWest()) && view.contain(bounds.getNorthEast())) {
        return
      }
    }
