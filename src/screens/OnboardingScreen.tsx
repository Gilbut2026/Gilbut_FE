 ...
 *  · walk·stairs·slope·rest·transfer·aid → MobilityProfileSaveRequest
 *  · voice → AccessibilitySetting(voiceGuidanceEnabled)
 *
 * 2026-08-12 BE 계약 반영
 *  · Backend MobilityProfileSaveRequest/OnboardingSaveRequest에서 slopeLevel이 필수값으로 추가됨.
 *  · 선택지는 괜찮음 / 조금 힘듦 / 많이 힘듦이며 각각 AVAILABLE /
 *    SLIGHTLY_DIFFICULT / DIFFICULT로 전송한다.
 *  · 이 값은 이후 AI route scoring의 사용자 경사 민감도에 사용된다.
 */
...
const QUESTIONS: Question[] = [
  ...
  { id: 'slope', icon: '⛰️', label: '오르막', title: '오르막길 이동은 어떠신가요?', help: '오르막길이 포함된 경로를 추천할 때 답변을 반영해요.', options: ['괜찮음', '조금 힘듦', '많이 힘듦'] },
...
export function OnboardingScreen({
  ...
  async function next() {
    ...
    setSaving(true)
    try {
      // 현재 FE dto.ts가 8/7 계약을 기준으로 하고 있어 slopeLevel 타입 반영이 늦더라도,
      // 실제 HTTP payload에는 Backend가 요구하는 slopeLevel을 반드시 포함한다.
      const profile = {
        walkingDuration: WALK_MAP[answers.walk],
        stairLevel: STAIR_MAP[answers.stairs],
        slopeLevel: SLOPE_MAP[answers.slope as keyof typeof SLOPE_MAP],
